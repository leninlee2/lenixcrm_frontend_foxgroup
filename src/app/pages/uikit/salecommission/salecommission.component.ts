...
@Component({
    ...
    templateUrl: './salecommission.component.html',
    styleUrl: './salecommission.component.css',
    providers: [MessageService]
})
export class SaleCommisionControlComponent  implements OnInit {
    id: string | null = null;
    ...
    resetGeneralSale(){
      this.salesGeneralControl = new SalesCommisionGeneralDTO(0,0,0,new Date(),true,'','',0,false
      , this.salesGeneralControl.datePeriod,0,0,'','',0,0);
    }
    ...
    editSalesGeneral(item:SalesCommisionGeneralDTO){
      this.salesGeneralControl=item;
      ...
      if(this.salesGeneralControl.hasPayment)
        this.salesGeneralControl.paymentDate = paymentDate;

      this.selectCustomer(item.idCustomer);
      console.log(item.salesman);
      this.selectSalesman(item.salesman);
      this.selectManager(item.manager);
      this.salesGeneralControl.specialPercent = this.salesGeneralControl.isManager?this.salesGeneralControl.percent:0;
    }
    ...
    addSalesGeneralControl(){
      
      this.salesGeneralControl.idCustomer = this.customer.id;
      this.salesGeneralControl.customer = this.customer.name;
      this.salesGeneralControl.idSalesman = this.salesman.id;
      this.salesGeneralControl.salesman = this.salesman.name;
      this.salesGeneralControl.idManager = this.manager.id;
      this.salesGeneralControl.manager = this.manager.name;
      console.log(this.salesGeneralControl);
      if(this.salesGeneralControl.id == 0){
        this.salesService.AddSalesGeneral(this.salesGeneralControl).subscribe(
          response => {
            this.getAllGeneralSales();
        ...
        this.salesService.UpdateSalesGeneral(this.salesGeneralControl).subscribe(
          response => {
            this.getAllGeneralSales();
            this.resetGeneralSale();
          },
    ...
    inactiveSalesGeneral(row:SalesCommisionGeneralDTO){
      this.salesService.InactiveSalesGeneral(row).subscribe(
        response => {
          this.getAllGeneralSales();
        },
        error => {
